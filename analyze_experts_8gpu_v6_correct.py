...
# Is this review positive or negative? ANSWER ONLY WITH Yes IF POSITIVE and No IF NEGATIVE. ONLY 1 word only Yes or No"""]
    ...
    for idx, prompt in enumerate(prompts):
        ...
        max_new = 2000  # Shorter for testing
        ...
        for step in range(max_new):
            with torch.no_grad():
                out = model(
                    ...
                    return_dict=True,
                    use_cache=False,  # Simpler for now
                )
            
            next_id = torch.argmax(out.logits[:, -1, :], dim=-1).unsqueeze(0)
            tok_id = int(next_id.item())
            tok_text = tokenizer.decode([tok_id], skip_special_tokens=True)
            
            # Extract routing for the last position
            ...
            for L in layer_experts:
                # Expert-level stats
                for e_id, weight in layer_experts[L]:
                    expert_counts[L][e_id] += 1
                    expert_mass[L][e_id] += weight
                    
                    # Group-level stats
                    g_id = e_id // experts_per_group
                    group_counts[L][g_id] += 1
                    group_mass[L][g_id] += weight
