...
def track_single_token_generation(prompts, expected_sentiment="unknown"):
    ...
    """
    # Structure: expert_counts[layer_idx][expert_idx] = count (total selections across all positions and prompts)
    expert_counts = defaultdict(lambda: defaultdict(int))
    # Structure: prompt_tally[layer_idx][expert_idx] = number of prompts where expert is used at least once
    prompt_tally = defaultdict(lambda: defaultdict(int))
    # List of per-prompt expert sets: list of dict[layer_idx: set of experts]
    prompt_expert_sets = []
    ...
    for idx, prompt in enumerate(prompts):
        ...
        messages = [
            {"role": "system", "content": "You are a helpful assistant that answers only with 'Yes' or 'No' for sentiment."},
                ...
                if hasattr(outputs, 'router_logits') and outputs.router_logits:
                    ...
                    for layer_idx, layer_logits in enumerate(outputs.router_logits):
                        if layer_logits is None:
                            continue
                        # Assume shape (batch, seq, experts)
                        ...
                        for pos in range(seq_len):
                            ...
                            for exp_idx in top_experts:
                                ...
                                layer_sets[layer_idx].add(exp_idx)
                            total_selections[layer_idx] += k
                    # After all layers, update prompt_tally and add to list
                    for layer_idx, exps in layer_sets.items():
                        for exp in exps:
                            prompt_tally[layer_idx][exp] += 1
                    prompt_expert_sets.append(layer_sets)
            except Exception as e:
                if idx == 0:
                    print(f" Note: Error during tracking - {str(e)[:50]}")
                predictions.append("Error")
    return expert_counts, total_selections, predictions, prompt_tally, prompt_expert_sets
def analyze_task_utilization(positive_counts, negative_counts, pos_predictions, neg_predictions, pos_prompt_tally, neg_prompt_tally, pos_prompt_expert_sets, neg_prompt_expert_sets):
    """
    ...
    for layer_idx, layer_counts in negative_counts.items():
        for expert_idx in layer_counts.keys():
            utilized_experts.add((layer_idx, expert_idx))
    # Combine prompt_tally
    combined_prompt_tally = defaultdict(lambda: defaultdict(int))
    for layer_idx, layer_tally in pos_prompt_tally.items():
        for exp, count in layer_tally.items():
            combined_prompt_tally[layer_idx][exp] += count
    for layer_idx, layer_tally in neg_prompt_tally.items():
        for exp, count in layer_tally.items():
            combined_prompt_tally[layer_idx][exp] += count
    # Combine prompt_expert_sets
    all_prompt_expert_sets = pos_prompt_expert_sets + neg_prompt_expert_sets
    total_prompts = len(all_prompt_expert_sets)
    ...
    # 5. Detailed Expert Activation per Layer (New section)
    print(f"\n" + "="*40)
    print("DETAILED EXPERT ACTIVATION PER LAYER")
    print("="*40)
    for layer_idx in range(num_hidden_layers):
        print(f"\nLayer {layer_idx:02d}:")
        # Get tally for this layer
        layer_tally = combined_prompt_tally.get(layer_idx, {})
        if not layer_tally:
            print("  No experts activated.")
            continue
        # Sort by tally descending
        sorted_experts = sorted(layer_tally.items(), key=lambda x: x[1], reverse=True)
        print("  Activated experts (Expert Index: tally of prompts where activated at least once):")
        for exp, tally in sorted_experts:
            print(f"    Expert {exp}: {tally}/{total_prompts} prompts")
        # Check if same across prompts
        layer_prompt_sets = [p.get(layer_idx, set()) for p in all_prompt_expert_sets]
        first_set = layer_prompt_sets[0]
        is_same = all(s == first_set for s in layer_prompt_sets)
        print(f"  Is the set of activated experts the same across all prompts? {'Yes' if is_same else 'No'}")
        if is_same:
            print(f"  Common set: {sorted(first_set)}")
        else:
            # Compute intersection (experts activated in all prompts)
            if layer_prompt_sets:
                intersection = set.intersection(*layer_prompt_sets)
                print(f"  Experts activated in all prompts: {sorted(intersection) if intersection else 'None'}")
    # 6. Identifying Unused Experts (Detailed List)
    print(f"\n" + "="*40)
...
if __name__ == "__main__":
    ...
    print("="*40)
    pos_counts, pos_totals, pos_predictions, pos_prompt_tally, pos_prompt_expert_sets = track_single_token_generation(
        positive_reviews,
    ...
    print("="*40)
    neg_counts, neg_totals, neg_predictions, neg_prompt_tally, neg_prompt_expert_sets = track_single_token_generation(
        negative_reviews,
        expected_sentiment="negative"
    )
    # Analyze utilization and pruning opportunities
    if pos_counts or neg_counts:
        analyze_task_utilization(pos_counts, neg_counts, pos_predictions, neg_predictions, pos_prompt_tally, neg_prompt_tally, pos_prompt_expert_sets, neg_prompt_expert_sets)
    else:
